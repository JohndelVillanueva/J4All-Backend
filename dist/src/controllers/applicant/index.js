...
const broadcastUpdate = (eventType, data, userId) => {
    const event = {
        type: eventType,
        data,
...
// Apply for a job
export const applyForJobController = async (c) => {
    try {
...
export const getUserApplicationsController = async (c) => {
    try {
...
export const getEmployerApplicantsController = async (c) => {
    try {
...
export const updateApplicationStatusController = async (c) => {
    try {
        ...
        try {
            ...
            const message = statusMessages[status.toUpperCase()] ||
...
export const realTimeUpdatesController = async (c) => {
    try {
        const authHeader = c.req.header('Authorization');
        if (!authHeader || !authHeader.startsWith('Bearer ')) {
        ...
        const stream = new ReadableStream({
            start(controller) {
                ...
                realTimeEvents.set(clientId, (event) => {
...
export const saveJobController = async (c) => {
    try {
...
export const unsaveJobController = async (c) => {
    try {
...
export const getSavedJobsController = async (c) => {
    try {
        ...
        const transformedSavedJobs = savedJobs.map(savedJob => {
            const job = jobMap.get(savedJob.job_id);
            ...
            return {
                ...
                skills: (job?.required_skills || []).map((rs) => ({
                    id: rs.skill.id,
                    name: rs.skill.name,
...
export const getApplicationDetailsController = async (c) => {
...
export const getApplicantProfileController = async (c) => {
    try {
...
export const checkSavedJobController = async (c) => {
    try {
