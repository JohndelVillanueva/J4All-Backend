...
export const getJobRecommendationsController = async (c) => {
    try {
...
export const generateJobRecommendations = async ({ userId, limit = 10, matchThreshold = 50 }) => {
    try {
...
const calculateJobMatch = (job, jobSeeker, userSkills) => {
    ...
    // 1. Skill matching
    const jobSkills = job.required_skills.map((rs) => ({
        id: rs.skill.id,
        name: rs.skill.name,
    ...
    const { matchedSkills, missingSkills, skillMatchPercentage } = calculateSkillMatch(userSkills, jobSkills);
    ...
    // 3. Salary matching
    const salaryMatch = checkSalaryMatch(job.salary_range_min, job.salary_range_max, jobSeeker.desired_salary);
    ...
    const experienceMatch = checkExperienceMatch(jobSeeker.experience_years, job.job_requirements);
...
const calculateSkillMatch = (userSkills, jobSkills) => {
    ...
    jobSkills.forEach((jobSkill) => {
        ...
        const userSkill = userSkills.find(us => us.name.toLowerCase() === jobSkill.name.toLowerCase());
        if (userSkill) {
...
const checkLocationMatch = (jobLocation, userPreference) => {
    if (!jobLocation || !userPreference)
        return true;
...
const checkSalaryMatch = (minSalary, maxSalary, desiredSalary) => {
    if (!desiredSalary)
        return true;
...
const checkExperienceMatch = (userExperience, jobRequirements) => {
    if (!userExperience || !jobRequirements)
        return true;
    // Simple keyword matching for experience requirements
    const requirements = jobRequirements.toLowerCase();
    if (requirements.includes("senior") && userExperience < 5)
        return false;
    if (requirements.includes("mid-level") && userExperience < 2)
        return false;
    if (requirements.includes("entry level") && userExperience > 3)
        return false;
    return true;
};
export const getRecommendedSkillsController = async (c) => {
    try {
        const user = c.get("user");
        if (!user?.id) {
            return c.json({
        ...
        const recommendedSkills = popularSkills.map(skill => {
            const activeJobRequiredSkills = skill.job_required_skills.filter((jrs) => jrs.job_listing?.is_active &&
            ...
            return {
                ...
                jobsCount: activeJobRequiredSkills.filter((jrs) => jrs.job_listing).length
            };
        });
        return c.json({
            success: true,
...
export const getSimilarJobsController = async (c) => {
    try {
...
const calculateJobSimilarity = (job1, job2) => {
    const skills1 = new Set(job1.required_skills.map((rs) => rs.skill.name));
    const skills2 = new Set(job2.required_skills.map((rs) => rs.skill.name));
    const intersection = new Set([...skills1].filter(skill => skills2.has(skill)));
    const union = new Set([...skills1, ...skills2]);
    return union.size > 0 ? (intersection.size / union.size) * 100 : 0;
};
export const getRecommendationStatsController = async (c) => {
    try {
        const user = c.get("user");
        if (!user?.id) {
            return c.json({
