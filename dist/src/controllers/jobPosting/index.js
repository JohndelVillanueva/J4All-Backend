...
export const createJobPostingController = async (c) => {
    ...
    // --- Rate limiting ---
    const rateLimitStore = global.jobPostingRateLimitStore || new Map();
...
export const getJobListingController = async (c) => {
    try {
        const user = c.get("user") || {
            id: c.get("userId"),
            userType: c.get("userType"),
...
export const getAllJobListingController = async (c) => {
    try {
        const jobListings = await prisma.jobListing.findMany({
            where: {
                is_active: true,
...
export const getJobPostingController = async (c) => {
    try {
...
export const updateJobPostingController = async (c) => {
    try {
        const employerId = c.get("employerId");
        const id = parseInt(c.req.param("id"));
...
export const deleteJobPostingController = async (c) => {
    try {
        const employerId = c.get("employerId");
        const id = parseInt(c.req.param("id"));
        // First verify the job listing belongs to this employer
...
export const deactivateExpiredJobsController = async (c) => {
    try {
        const user = c.get("user");
