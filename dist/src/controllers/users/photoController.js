 ...
 */
export const uploadUserPhotoController = async (c) => {
    try {
        ...
        // Upload the photo
        const result = await PhotoService.updateUserPhoto(Number(user.id), photoFile, photoFile.name);
        return c.json({
            success: true,
 ...
 */
export const deleteUserPhotoController = async (c) => {
    try {
        const user = c.get('user') || {
            id: c.get('userId'),
            userType: c.get('userType'),
 ...
 */
export const getUserPhotoController = async (c) => {
    try {
 ...
 */
export const updateUserPhotoBase64Controller = async (c) => {
    try {
        const user = c.get('user') || {
            id: c.get('userId'),
            userType: c.get('userType'),
        ...
        // Upload the photo
        const result = await PhotoService.updateUserPhoto(Number(user.id), buffer, filename);
        return c.json({
            success: true,
