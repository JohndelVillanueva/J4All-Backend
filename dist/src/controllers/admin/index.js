...
export const getAdminStatsController = async (c) => {
    try {
...
export const getAdminUsersListController = async (c) => {
    try {
...
export const getAdminEmployersListController = async (c) => {
    try {
        const url = new URL(c.req.url);
        const page = Math.max(1, Number(url.searchParams.get('page') || '1'));
        const pageSize = Math.max(1, Math.min(100, Number(url.searchParams.get('pageSize') || '10')));
...
export const getAdminAnalyticsController = async (c) => {
    try {
        const now = new Date();
        const monthsBack = 12;
        ...
        users.forEach(u => {
            ...
            if (buckets[k]) {
                ...
                if (u.user_type === 'employer')
                    buckets[k].employer += 1;
                if (u.user_type === 'general')
                    buckets[k].general += 1;
                if (u.user_type === 'pwd')
                    buckets[k].pwd += 1;
            }
...
export const getAdminActivitiesController = async (c) => {
    try {
        const url = new URL(c.req.url);
        const page = Math.max(1, Number(url.searchParams.get('page') || '1'));
        ...
        const idToUser = Object.fromEntries(users.map(u => [u.id, (u.first_name && u.last_name ? `${u.first_name} ${u.last_name}` : (u.username || u.email || String(u.id)))]));
        const activities = [];
        ...
        const mapStatus = (s) => {
            ...
            if (val === 'HIRED' || val === 'APPROVED' || val === 'ACCEPTED')
                return 'approved';
            if (val === 'REJECTED' || val === 'DECLINED')
                return 'declined';
            return 'applied';
        };
        ...
        const convToSenders = new Map();
        grouped.forEach(g => {
            if (!convToSenders.has(g.conversation_id))
                convToSenders.set(g.conversation_id, new Map());
